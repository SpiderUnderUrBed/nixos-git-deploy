...
func writer(pipeFile string, origin string, messages chan string, wg *sync.WaitGroup) {
	// Defer the decrement of the wait group counter
	defer wg.Done()
	//fmt.Println("writing")
...
func runChildProcess() {
	...
	go writer("detach.log", "child", messages, &wg)
...
func killprocess(Pid int) {
    ...
    if err != nil {
        fmt.Println("Error sending SIGTERM signal:", err)
        return
    }
}
func main() {
	...
	go writer("recede.log", "parent", messages, &wg)
		...
		case "add-automatic":
			...
			if git, err := git.PlainOpen(gitDirectory); err == nil {
				...
				for _, file := range files {
					...
					// Send the file name through the channel
					messages <- file
